    ...
    switch (msg.type) {
        case 'session_status':
            if (msg.valid) {
                ...
                // Use msg.name if provided, otherwise fall back to localStorage
        ...
        case 'pin_verified':
            if (msg.success) {
                authState.authenticated = true;
                authState.playerName = document.getElementById('authLoginNameInput').value.trim();
                authState.sessionToken = msg.sessionToken;
                authState.inventory = msg.inventory || getEmptyInventory();
                localStorage.setItem('item_session', msg.sessionToken);
                localStorage.setItem('boggle_playerName', authState.playerName);
                ...
                showAuthError('authLoginError', 'Wrong name or PIN');
                authState.loginPin = '';
